...
export async function getHelpBoard(repo: HelpFlagRepositoryPort, db: DbClient, context: HelpFlagContext) {
  ...
  const raiseLevel = level && !existing && crewReady && (level === 2 || workload.length) ? level : null;
  const rows = [];
  for (const flag of flags) {
    const canEscalate = context.actorRole === 'PARTY_CHIEF' && flag.level === 1 && !existing &&
      await repo.crewChief(db, context.tenantId, context.projectId, flag.raisedBy) === context.actorId &&
      !await repo.findEscalation(db, context.tenantId, context.projectId, flag.id);
    rows.push({ ...flag, canClaim: context.actorRole === 'PARTY_CHIEF' && flag.level === 2 && flag.affectedTicketIds.length > 0 && flag.raisedBy !== context.actorId, own: flag.raisedBy === context.actorId,
