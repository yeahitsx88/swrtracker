    ...
    const { rows } = await db.query<TicketLabels>(
      `WITH RECURSIVE location AS (
       ...
       SELECT p.name AS "projectName",
         ...
         su.name AS "superintendentName", ca.name AS "cadAssigneeName", cr.name AS "cadReviewerName"
       ...
       LEFT JOIN users ca ON ca.id=$8 AND ca.tenant_id=p.tenant_id
       LEFT JOIN users cr ON cr.id=$9 AND cr.tenant_id=p.tenant_id
      ...
      [context.tenantId, context.projectId, context.aorNodeId, context.departmentId,
        ...
        context.cadAssigneeId ?? null, context.cadReviewerId ?? null]);
    return rows[0] ?? null;
  }
}
