  ...
  return withRequestCorrelation(req, async () => {
    try {
      ...
      const result = await withTransaction((db) => executeIdempotentHttpMutation(
        db,
        { tenantId: ctx.tenantId, actorId: ctx.actorId, endpoint: `POST:/api/tickets/${ticketId}/return`, idempotencyKey },
        { ticketId, reason: body.reason },
        async () => {
          const ticket = await returnTicketForCorrection(repo, db, {
            ...ctx,
            reason: body.reason as string,
            origin: 'SURVEY_CHANGE',
          });
