...
interface TicketPage { data: RequestTicket[]; total: number; limit: number; offset: number; canRequest: boolean; canViewHelpFlags: boolean }
...
export function RequestList({ projectId }: { projectId: string }) {
  ...
  return <Shell projectId={projectId} signedIn={Boolean(page)}><p className="eyebrow">Project workspace</p><h1>Requests</h1>
    ...
    <div className="actions">{page?.canRequest && <Link className="button" href={`/project/${projectId}/request`}>New request</Link>}<Link href="/drafts">My drafts</Link>{page?.canViewHelpFlags && <Link href={`/project/${projectId}/help`}>Crew help flags</Link>}</div>
