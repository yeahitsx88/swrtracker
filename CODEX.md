...
### 2026-09-25 - Batch 51
- Intent: implement the tenant-admin audit query/API required by CLAUDE.md Section 23.
- Files touched: src/modules/audit/application/audit-log.ts; src/modules/audit/infrastructure/audit-log.repository.ts; src/app/api/audit/route.ts; db/migrations/025_draft_audit_project_context.sql; tests/audit/audit-log.test.ts; CODEX.md.
- Behavior added: GET /api/audit returns bounded pages combining ticket_events and tenant_events with complete payloads, actor/project names, and exact project/event/actor plus UTC date-window filters (from inclusive, until exclusive). Tenant-admin authorization is checked at the application layer and again in the data query. Joins preserve archived-project, deactivated-actor, general-help, and purged-draft events. Response caching disabled.
- Migration: 025 records the original project on new draft tombstones before deletion, preserving project filtering without changing historical audit rows; adds tenant chronology indexes for both event tables. Applied and rerun successfully on configured dev database. Existing tombstones lacking project context remain readable in the unfiltered log; their original project cannot be recovered unless an event payload records it.
- Verification: baseline TypeScript and pnpm test passed (151 pass, 32 skips). Final TypeScript and production build passed; pnpm test passed (152 pass, 33 database skips); PostgreSQL suite passed (183 pass, 2 dedicated-URL skips). Tests cover role/tenant denial, repository authorization, stable pagination, exact filters/date boundaries, archived project visibility, full payloads, historical actor names, and project filtering after real draft purge.
...
### 2026-09-25 - Batch 53
- Intent: deliver the tenant audit viewer and filtered CSV download surface.
- Files touched: src/app/audit/page.tsx; src/app/audit/audit-picker.tsx; src/app/api/audit/route.ts; src/app/projects/page.tsx; src/app/ui/api.ts; src/modules/audit/application/audit-filters.ts; src/modules/audit/infrastructure/audit-filters.repository.ts; src/modules/tenancy/application/project-directory.ts; tests/audit/audit-log.test.ts; tests/tenancy/project-directory.test.ts; tests/ticket/request-ui.test.ts; CODEX.md.
- Behavior added: /audit displays paginated tenant activity with expandable full payloads, named searchable project/actor/event choices, local date-window inputs, applied-filter CSV download, loading/empty/error states, and login return. Projects exposes the audit link only for tenant admins. Filter option queries enforce tenant/admin scope and retain inactive actors. Signed-in non-admins see a clear access message and Sign out, with no audit data or controls.
- Verification: TypeScript, production build, pnpm test (158 pass, 34 database skips), PostgreSQL suite (190 pass, 2 dedicated-URL skips), and diff whitespace checks passed. Tests extend tenant/admin option isolation, pagination, literal search, historical actors, navigation capability, and safe login return.
- Browser acceptance: isolated fixture migrated through 025; verified audit login return, 20-row paging, archived project and actor filters, event choices, date-filter empty result, payload expansion, admin navigation, and non-admin denial/no audit link. Downloaded filtered CSV read back from Downloads: exactly one matching draft event, correct project and full nested payload. The browser download notification timed out, but the actual saved CSV was independently verified. Corrected signed-in denial navigation and verified after rebuild. Preview server stopped, tab closed, fixture database and credential files removed. Screenshot .local/audit-viewer-verified.png retained locally.
...
### 2026-09-25 - Batch 60
- Intent: provide historical daily activity through the Ticket application boundary (IMPLEMENTER, Ticket module).
- Files touched: src/modules/ticket/application/daily-report.ts; src/modules/ticket/application/daily-report-events.ts; src/modules/ticket/application/operational-report.ts; src/modules/ticket/application/index.ts; src/modules/ticket/infrastructure/ticket.repository.ts; tests/ticket/operational-report.test.ts; CODEX.md.
- Behavior added: daily ticket activity reads immutable ticket_events within inclusive start/exclusive end UTC boundaries, at most 25 hours to accommodate local daylight-saving days. Returns distinct requests and event occurrence counts for fourteen existing operational event types. Direct creation is distinguished from standard draft creation; drafts and unrelated audit events are excluded. Existing report role resolution and repository visibility filters scope historical activity to currently visible requests, including archived projects. Shared report authorization was extracted within the module for the two report services.
- Verification: baseline TypeScript and pnpm test passed (162 pass, 36 skips). Final TypeScript/build passed; pnpm test (163 pass, 36 skips) and PostgreSQL suite (197 pass, 2 dedicated-URL skips) passed. Focused integration rerun after additional historical-status/deactivation assertions passed. Tests cover invalid/oversized/reversed bounds, start/end boundaries, 23/25-hour ranges, empty day, repeated assignment occurrences vs distinct requests, direct-vs-draft creation, other-project/unrelated-event exclusion, requester/PC/IM scope, wrong tenant, project-admin denial, deactivated user and activity surviving later status change. All fixture changes roll back.
- Module boundary deviations: none. No event names, migrations or HTTP routes added. Daily report events are data-only constants to avoid introducing runtime import cycles.
- Known gaps queued for later batches: daily Reporting orchestration/API/UI and browser date-boundary acceptance; grouped reporting volume measurements; duplicate area name differentiation. Automatic daily distribution has no specified recipient/cadence contract and remains unimplemented.
- Production behavior changed: new Ticket application service; HTTP/UI exposure follows in a later batch.

### 2026-09-25 - Batch 61
- Intent: deliver on-demand daily summaries through Reporting, the existing report API, and the report date interface (IMPLEMENTER, Reporting).
- Files touched: src/modules/reporting/application/daily-summary.ts; src/modules/reporting/application/index.ts; src/app/api/projects/[projectId]/reports/route.ts; src/app/project/[projectId]/reports/daily-summary.tsx; src/app/project/[projectId]/reports/report-day.ts; src/app/project/[projectId]/reports/project-reports.tsx; src/app/project/[projectId]/reports/reports.module.css; tests/reporting/daily-summary.test.ts; tests/reporting/report-day.test.ts; CODEX.md.
- Behavior added: reports API view=daily requires exact from/until boundaries and rejects incompatible grouping/pagination. Reporting consumes the authorized Ticket service, supplies fourteen stable plain-language rows including zero values, separates distinct requests from event occurrences, and totals occurrences only. Daily summary selector uses browser-local dates converted to UTC midnight boundaries, refreshes activity, explains visibility/count semantics, and displays quiet/loading/error states. Existing current report grouping remains available.
- Verification: baseline TypeScript and pnpm test passed (163 pass, 36 skips). Final TypeScript, production build, pnpm test (167 pass, 36 skips), and PostgreSQL suite (201 pass, 2 dedicated-URL skips) passed. New tests cover summary mapping, zero rows, failure propagation, invalid dates, year/leap-day rollover, and America/Chicago 23-hour spring and 25-hour autumn day conversion.
- Browser acceptance: isolated migrated fixture showed 15 daily events with one request/two assignment events and exact local midnight boundaries. Date keyboard change produced a quiet day with all zero rows; switching to current report and back worked. Browser date fill alone did not trigger React change, so acceptance used the native date keyboard control. Mobile viewport 390x844 had matching 375px content/client widths; screenshot .local/daily-summary-mobile.png retained. Preview stopped, viewport reset, tab closed, fixture database and credential files removed.
