...
test('CAD summary authorizes ticket visibility before retrieving CAD and detects duplicate records',async()=>{
  let reads=0;
  const cad={async find(_db:DbClient,tenantId:UUID,key:UUID){assert.equal(tenantId,tenant);assert.equal(key,ticketId);reads++;return [{status:'QA_PENDING' as const,completedAt:null}];}};
  ...
  assert.deepEqual(await getCadSummary(visible,cad,db,params),{status:'QA_PENDING',completedAt:null});
  assert.equal(await getCadSummary(visible,{find:async()=>[]},db,params),null);
  await assert.rejects(getCadSummary(visible,{find:async()=>[{status:'NOT_REQUIRED',completedAt:null},{status:'COMPLETE',completedAt:new Date()}]},db,params),ConflictError);
...
test('PostgreSQL CAD summary is tenant/ticket scoped and returns recorded status and completion',
  {skip:!process.env.DATABASE_URL},async()=>{
    ...
    try {
      ...
      assert.deepEqual(await repo.find(client,tenant,ticketId),[{status:'COMPLETE',completedAt:new Date('2026-09-25T12:00:00Z')}]);
